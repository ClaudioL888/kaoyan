...
- [ ] 协议层次、通信实体、服务对象、作用范围及不负责事项（待具体复习本章时核对）
- [ ] 报文字段的产生方、消费方、单位和逐跳/端到端变化（待具体复习本章时核对）
- [ ] 主机、交换机、路由器、NAT 和服务器的不同视角（待具体复习本章时核对）
- [ ] ARP/交换/路由/NAT 表及 TCP 状态的建立、更新和失效（待具体复习本章时核对）
- [ ] 消息方向、定时器、状态机、重传与失败路径（待具体复习本章时核对）
- [ ] 协议版本、历史机制、公式模型和拓扑缺失信息边界（待具体复习本章时核对）
