...
- [ ] ADT 与具体存储实现的分层（待具体复习本章时核对）
- [ ] 数组、指针、栈、队列、树和图的中间状态（待具体复习本章时核对）
- [ ] 伪代码下标、哨兵、前驱、递归返回值及副作用约定（待具体复习本章时核对）
- [ ] 循环/递归不变量、终止性与正确性依据（待具体复习本章时核对）
- [ ] 最好/平均/最坏复杂度、辅助空间、稳定性与原地性（待具体复习本章时核对）
- [ ] 比较表中的已定位、连通、关键字互异等隐含前提（待具体复习本章时核对）
