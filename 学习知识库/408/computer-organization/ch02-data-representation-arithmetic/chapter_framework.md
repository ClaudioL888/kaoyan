...
- [ ] 位、字节、字、字长、存储字和地址空间的单位层级（待具体复习本章时核对）
- [ ] 公式推导、量纲、平均/峰值口径与成立前提（待具体复习本章时核对）
- [ ] 数据通路、控制信号、并行部件和关键路径（待具体复习本章时核对）
- [ ] 指令/地址/Cache/页表/浮点格式的字段依据（待具体复习本章时核对）
- [ ] 时序、周期数、流水线冒险及解决代价（待具体复习本章时核对）
- [ ] ISA—CPU—Cache—主存—I/O 的跨章数据链（待具体复习本章时核对）
